...
import { SignInI, useGetUser, useSignIn } from "./api";
...
import { getDecodedValue } from "../../redux/slices/auth";
...
const SignIn = () => {
    ...
    const { handleSubmit, handleChange, values, errors, touched } = useFormik<SignInI>({
      initialValues,
      ...
      onSubmit: () => {
        signinMutation.mutate(values, {
          onSuccess: (data) => {
            ...
            dispatch(getDecodedValue(data.user));
          },
          onError: (error) => {
    ...
    <main className=' bg-amber-50/20 h-screen flex flex-col md:flex-row md:justify-center md:items-center'>
        <div className="flex flex-col md:flex-row h-full md:justify-center md:items-center">
            ...
            <section className="flex flex-col mt-2 items-end">
                <p className=" text-sm text-[#fa9b0a] ">Don't have an account <Link to={RoutePaths.SIGNUP}><span className="font-bold text-black cursor-pointer">SignUp</span></Link></p>
            </section>
            <section>
                
                <button disabled={signinMutation.isPending} type="submit" className="bg-black cursor-pointer text-white px-4 py-3 rounded-xl w-full mt-5">
                    {signinMutation.isPending ? 'Signin...' : 'Login'}
                </button>
